...
## ملفات البناء الثلاثة

| الملف | الناتج | يشتغل من غير الكمبيوتر؟ | الاستخدام |
|---|---|---|---|
| `development` | APK + dev client | ❌ محتاج `expo start` شغال | التطوير اليومي — تعديل الكود بيظهر فورًا |
| `preview` | **APK مستقل** | ✅ | تجربة التطبيق زي المستخدم · تبعته لأي حد يجرّبه |
| `production` | `.aab` | — | رفعه على Google Play بس. ما ينفعش يتثبّت مباشرة |

### بناء APK مستقل

```powershell
git add -A
git commit -m "chore: build config"
npx eas-cli@latest build --profile preview --platform android
```

بعد ما يخلص، امسح الـ QR من كاميرا التليفون، نزّل الـ APK، وثبّته. التطبيق هيشتغل
لوحده تمامًا — مش محتاج الكمبيوتر ولا شبكة محلية.

> 📌 **الفرق المهم:** في `preview` الكود بيتحزّم جوه الـ APK. يعني أي تعديل تعمله
> بعد كده **مش هيظهر** إلا لما تبني نسخة جديدة. لو بتطوّر وبتجرّب كل شوية، استخدم
> `development` — أسرع بكتير.

> 🔑 متغيّرات `EXPO_PUBLIC_*` بتتحقن وقت البناء من `eas.json` (مش من `.env`، لأنه
> مستبعد من git). لو غيّرت مشروع Supabase، عدّل `eas.json`.
